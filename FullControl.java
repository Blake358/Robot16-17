...
public class FullControl extends OpMode {
    ...
    double maximumLauncherPower = 0.8;
    ...
    final static double leftBeaconArmDownPosition = 0.901; // REMEMBER: These values need to be recalibrated
    final static double leftBeaconArmUpPosition = 0.345;
    final static double rightBeaconArmDownPosition = 0.01;
    final static double rightBeaconArmUpPosition = 0.64;
    ...
    public void loop() {
        ...
        if (gamepad1.left_bumper) {
            if (!leftBeaconArmState && previousLeftBeaconArmState) { // This section is all new code, so please make sure it works correctly
                leftBeaconArmState = true;
                previousLeftBeaconArmState = false;
            }

            if (leftBeaconArmState && !previousLeftBeaconArmState) {
                leftBeaconArmState = false;
                previousLeftBeaconArmState = true;
            }
        }

        if (gamepad1.right_bumper) {
            if (!rightBeaconArmState && previousRightBeaconArmState) {
                rightBeaconArmState = true;
                previousRightBeaconArmState = false;
            }

            if (rightBeaconArmState && !previousRightBeaconArmState) {
                rightBeaconArmState = false;
                previousRightBeaconArmState = true;
            }
        }
